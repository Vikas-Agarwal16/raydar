        ...
        (res) => {
          ...
          res.on("end", () => {
            try {
              ...
              reject(new Error("Amazon search.json did not return valid JSON — endpoint may have changed"));
            }
          });
        }
      )
...
async function scrapeAmazonInternship(baseQuery) {
  const encodedQuery = encodeURIComponent(baseQuery);
  const searchUrl = `https://www.amazon.jobs/en/search.json?base_query=${encodedQuery}&country=IND&loc_query=India&result_limit=10&sort=recent&offset=0`;
  const refererUrl = `https://www.amazon.jobs/en/search?base_query=${encodedQuery}&country=IND`;

  const data = await fetchJSON(searchUrl, refererUrl);

  if (!data.jobs || data.jobs.length === 0) {
    // Not an error — the query and location filter both worked correctly,
    // there's just genuinely no matching listing in India right now.
    // Distinct from a thrown error, which should mean "scraper broke."
    return { title: null, url: null, severity: "WATCHING" };
  }
  ...
  return {
    title: latest.title,
    url: `https://www.amazon.jobs${latest.job_path}`,
    severity: classifySeverity(latest.title),
  };
}

export const scrapeAmazonSDEIntern = () =>
  scrapeAmazonInternship("software development engineer intern");
export const scrapeAmazonDataScientistIntern = () =>
  scrapeAmazonInternship("data scientist intern");
export const scrapeAmazonMLIntern = () =>
  scrapeAmazonInternship("machine learning intern");


async function scrapeInternshala(categorySlug) {
  const html = await fetchHTML(`https://internshala.com/internships/${categorySlug}-internship/`);
  const $ = cheerio.load(html);

  const internships = [];

  $('a[href*="/internship/detail/"]').each((i, el) => {
    const title = $(el).text().trim();
    const href = $(el).attr("href");
    if (title && href) {
      internships.push({ title, url: href });
  ...
  if (internships.length === 0) {
    throw new Error("No internships found — Internshala's HTML structure may have changed");
  }
  ...
  return {
    title: latest.title,
    url: latest.url.startsWith("http") ? latest.url : `https://internshala.com${latest.url}`,
    severity: classifySeverity(latest.title),
  };
}

export const scrapeInternshalaCS = () => scrapeInternshala("computer-science");
export const scrapeInternshalaWebDev = () => scrapeInternshala("web-development");


export async function scrapeCBSE() {
  const html = await fetchHTML("https://www.cbse.gov.in/cbsenew/cbse.html");
  const $ = cheerio.load(html);

  const notices = [];

  $('a[target="_blank"]').each((i, el) => {
    if ($(el).find("i.fa-file-pdf-o").length === 0) return;
  ...
  if (notices.length === 0) {
    throw new Error("No notices found — CBSE's HTML structure may have changed");
  }
  ...
  return {
    title: latest.title,
    url: latest.url.startsWith("http") ? latest.url : `https://www.cbse.gov.in/cbsenew/${latest.url}`,
    severity: classifySeverity(latest.title),
  };
}


export async function scrapeIPU() {
  const html = await fetchHTML("https://ipu.admissions.nic.in/");
  const $ = cheerio.load(html);

  const noticesBlock = $("h2")
    .filter((i, el) => $(el).text().trim() === "NEWS & EVENTS")
    .closest(".gen-list");

  const notices = [];
  noticesBlock.find("ul li a").each((i, el) => {
    const title = $(el).text().trim().replace(/\s+/g, " ");
    const href = $(el).attr("href");
    if (title && href) {
      notices.push({ title, url: href });
  ...
  if (notices.length === 0) {
    throw new Error("No notices found — IPU's HTML structure may have changed");
  }
...
const SCRAPERS = {
  ...
  'amazon-sde-intern': scrapeAmazonSDEIntern,
  'amazon-data-scientist-intern': scrapeAmazonDataScientistIntern,
  'amazon-ml-intern': scrapeAmazonMLIntern,
  'internshala-computer-science': scrapeInternshalaCS,
  'internshala-web-development': scrapeInternshalaWebDev,
  cbse: scrapeCBSE,
  ipu: scrapeIPU,
};
